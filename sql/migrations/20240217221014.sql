...
CREATE TABLE `new_pieces` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `0` FOREIGN KEY (`mode_id`) REFERENCES `scale_modes` (`id`) ON UPDATE NO ACTION ON DELETE SET NULL,
  CONSTRAINT `1` FOREIGN KEY (`key_id`) REFERENCES `scale_keys` (`id`) ON UPDATE NO ACTION ON DELETE SET NULL,
...
INSERT INTO `new_pieces` (`id`, `title`, `description`, `composer`, `measures`, `beats_per_measure`, `goal_tempo`, `user_id`, `last_practiced`, `stage`) SELECT `id`, `title`, `description`, `composer`, `measures`, `beats_per_measure`, `goal_tempo`, `user_id`, `last_practiced`, `stage` FROM `pieces`;
