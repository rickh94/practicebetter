...
CREATE TABLE `pieces` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `0` FOREIGN KEY (`composer_id`) REFERENCES `composers` (`id`) ON UPDATE NO ACTION ON DELETE SET NULL
);
...
CREATE TABLE `items` (
  ...
  `publisher` text NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `0` FOREIGN KEY (`box_id`) REFERENCES `boxes` (`id`) ON UPDATE NO ACTION ON DELETE SET NULL
);
-- Create index "item_box" to table: "items"
CREATE INDEX `item_box` ON `items` (`box_id`);
-- Create index "item_description" to table: "items"
CREATE INDEX `item_description` ON `items` (`description`);
-- Create index "item_instrument" to table: "items"
CREATE INDEX `item_instrument` ON `items` (`instrument`);
-- Create index "item_publisher" to table: "items"
...
CREATE TABLE `items_to_pieces` (
  ...
  CONSTRAINT `1` FOREIGN KEY (`item_id`) REFERENCES `items` (`id`) ON UPDATE NO ACTION ON DELETE CASCADE
);
